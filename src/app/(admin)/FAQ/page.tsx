import React from "react";
import { LuPlus } from "react-icons/lu";

const FAQ = () => {
  return (
    <section className="text-gray-600 body-font font-[Inter] items-center justify-center w-[800px] md:w-fit ml-10">
      <div className="container px-5 py-24 mx-auto">
        <div className="flex flex-wrap w-full mb-20 flex-col items-center text-center">
          <h1 className="sm:text-3xl text-2xl font-bold title-font mb-2 text-[#333333]">
            Questions Looks Here
          </h1>
          <p className="lg:w-1/2 w-full leading-relaxed text-[#4F4F4F] font-normal">
            Lorem Ipsum is simply dummy text of the printing and typesetting
            industry. Lorem Ipsum has been the
          </p>
        </div>
        <div className="flex flex-wrap -m-4 mx-5 mb-4">
          <div className="xl:w-1/2 p-4">
            <div className="border border-gray-200 bg-[#F7F7F7] p-6 rounded-lg transition-all duration-300 ease-in-out hover:bg-[#e4e4e4] hover:scale-105">
              <div className="flex justify-between">
                <h2 className="text-lg text-[#333333] font-bold title-font mb-2">
                  What types of chairs do you offer?
                </h2>
                <LuPlus className="text-[#333333] h-[24px] w-[24px]" />
              </div>
              <p className="leading-relaxed text-base font-normal text-[#4F4F4F]">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Nisi
                quis modi ullam amet debitis libero veritatis enim repellat
                optio natus eum delectus deserunt, odit expedita eos molestiae
                ipsa totam quidem?
              </p>
            </div>
          </div>
          <div className="xl:w-1/2 p-4">
            <div className="border border-gray-200 bg-[#F7F7F7] p-6 rounded-lg transition-all duration-300 ease-in-out hover:bg-[#e4e4e4] hover:scale-105">
              <div className="flex justify-between">
                <h2 className="text-lg text-[#333333] font-bold title-font mb-2">
                  How can we get in touch with you?
                </h2>
                <LuPlus className="text-[#333333] h-[24px] w-[24px]" />
              </div>
              <p className="leading-relaxed text-base font-normal text-[#4F4F4F]">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Nisi
                quis modi ullam amet debitis libero veritatis enim repellat
                optio natus eum delectus deserunt, odit expedita eos molestiae
                ipsa totam quidem?
              </p>
            </div>
          </div>
          <div className="xl:w-1/2 p-4">
            <div className="border border-gray-200 bg-[#F7F7F7] p-6 rounded-lg transition-all duration-300 ease-in-out hover:bg-[#e4e4e4]  hover:scale-105">
              <div className="flex justify-between">
                <h2 className="text-lg text-[#333333] font-bold title-font mb-2">
                  Do your chairs come with a warranty?
                </h2>
                <LuPlus className="text-[#333333] h-[24px] w-[24px]" />
              </div>
              <p className="leading-relaxed text-base font-normal text-[#4F4F4F]">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Nisi
                quis modi ullam amet debitis libero veritatis enim repellat
                optio natus eum delectus deserunt, odit expedita eos molestiae
                ipsa totam quidem?
              </p>
            </div>
          </div>
          <div className="xl:w-1/2 p-4">
            <div className="border border-gray-200 bg-[#F7F7F7] p-6 rounded-lg transition-all duration-300 ease-in-out hover:bg-[#e4e4e4] hover:scale-105">
              <div className="flex justify-between">
                <h2 className="text-lg text-[#333333] font-bold title-font mb-2">
                  What will be delivered? And When?
                </h2>
                <LuPlus className="text-[#333333] h-[24px] w-[24px]" />
              </div>
              <p className="leading-relaxed text-base font-normal text-[#4F4F4F]">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Nisi
                quis modi ullam amet debitis libero veritatis enim repellat
                optio natus eum delectus deserunt, odit expedita eos molestiae
                ipsa totam quidem?
              </p>
            </div>
          </div>
          <div className="xl:w-1/2 p-4">
            <div className="border border-gray-200 bg-[#F7F7F7] p-6 rounded-lg transition-all duration-300 ease-in-out hover:bg-[#e4e4e4]  hover:scale-105">
              <div className="flex justify-between">
                <h2 className="text-lg text-[#333333] font-bold title-font mb-2">
                  Can I try a chair before purchasing?
                </h2>
                <LuPlus className="text-[#333333] h-[24px] w-[24px]" />
              </div>
              <p className="leading-relaxed text-base font-normal text-[#4F4F4F]">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Nisi
                quis modi ullam amet debitis libero veritatis enim repellat
                optio natus eum delectus deserunt, odit expedita eos molestiae
                ipsa totam quidem?
              </p>
            </div>
          </div>
          <div className="xl:w-1/2 p-4">
            <div className="border border-gray-200 bg-[#F7F7F7] p-6 rounded-lg transition-all duration-300 ease-in-out hover:bg-[#e4e4e4]  hover:scale-105">
              <div className="flex justify-between">
                <h2 className="text-lg text-[#333333] font-bold title-font mb-2">
                  How do I clean and maintain my Comforty chair?
                </h2>
                <LuPlus className="text-[#333333] h-[24px] w-[24px]" />
              </div>
              <p className="leading-relaxed text-base font-normal text-[#4F4F4F]">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Nisi
                quis modi ullam amet debitis libero veritatis enim repellat
                optio natus eum delectus deserunt, odit expedita eos molestiae
                ipsa totam quidem?
              </p>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default FAQ;
